...
def RunGeminiMacro(model_id, prompt, temperature, language, system_prompt=None):
  """Runs a Gemini macro.
...
def RunMacro(macro_id, user_inputs, temperature, model_id):
  ...
  # Extract system prompt from user inputs
  system_prompt = user_inputs.get('systemPrompt', '')

  return RunGeminiMacro(model_id, prompt, temperature, language, system_prompt)
