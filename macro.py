...
def RunGeminiMacro(model_id, prompt, temperature, language):
  ...
  api_key = secrets_helper.get_secret('GEMINI_API_KEY')
  if not api_key:
    api_key = secrets_helper.get_secret('API_KEY')
    if api_key:
      logger.warning(
          'Using deprecated API_KEY env var. Please migrate to GEMINI_API_KEY.')
